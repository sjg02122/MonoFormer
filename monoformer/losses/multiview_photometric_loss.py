...
from monoformer.utils.image import match_scales
from monoformer.geometry.camera import Camera
from monoformer.geometry.camera_utils import view_synthesis
from monoformer.utils.depth import calc_smoothness, inv2depth
from monoformer.losses.loss_base import LossBase, ProgressiveScaling
...
########################################################################################################################
    ...
    def warp_ref_image(self, inv_depths, ref_image, K, ref_K, pose):
        ...
        # wandb.log({"Ref_img":wandb.Image(ref_images[0],caption="Ref_Images")})
        ref_warped = [view_synthesis(
            ref_images[i], depths[i], ref_cams[i], cams[i],
            padding_mode=self.padding_mode) for i in range(self.n)]
        #wandb.log({"Ref_warped":wandb.Image(ref_warped[0],caption='Ref_warped_Images')})
...
########################################################################################################################
    ...
    def reduce_photometric_loss(self, photometric_losses):
        ...
        wandb.log({"photometric_loss":photometric_loss})
...
########################################################################################################################
    ...
    def calc_smoothness_loss(self, inv_depths, images):
        ...
        wandb.log({"Smooth Loss":smoothness_loss})
