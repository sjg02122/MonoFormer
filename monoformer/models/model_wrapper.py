...
from monoformer.datasets.transforms import get_transforms
from monoformer.utils.depth import inv2depth, post_process_inv_depth, compute_depth_metrics
from monoformer.utils.horovod import print0, world_size, rank, on_rank_0
from monoformer.utils.image import flip_lr
from monoformer.utils.load import load_class, load_class_args_create, \
    load_network, filter_args
from monoformer.utils.logging import pcolor
from monoformer.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics
from monoformer.utils.save import save_depth
from monoformer.models.model_utils import stack_batch
...
def setup_depth_net(config, prepared, **kwargs):
    ...
    """
    print0(pcolor('DepthNet: %s' % config.name+'_'+config.backbone, 'yellow'))
    depth_net = load_class_args_create(config.name,
        paths=['monoformer.networks.depth',],
        args={**config, **kwargs},
    )
...
def setup_pose_net(config, prepared, **kwargs):
    ...
    pose_net = load_class_args_create(config.name,
        paths=['monoformer.networks.pose',],
        args={**config, **kwargs},
    )
    if not prepared and config.checkpoint_path is not '':
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        if config.dataset[i] == 'KITTI':
            from monoformer.datasets.kitti_dataset import KITTIDataset
        ...
        # DGP dataset
            ...
            from monoformer.datasets.dgp_dataset import DGPDataset
            dataset = DGPDataset(
        ...
        # Image dataset
            ...
            from monoformer.datasets.image_dataset import ImageDataset
